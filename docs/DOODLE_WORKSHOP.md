...
Frame the paper with the four corner dots, adjust paper-removal sensitivity, and use **Keep** and **Erase** to correct the transparent preview. This step removes page white; it does not detect limbs or make cutouts for you. In **Bring to life**, choose **Trace cutouts** and tap around the outside edge of each body part (head, body, two arms and two legs). Use enough points to follow the whole part; tiny or nearly straight slivers are rejected. The full source photo stays visible in the preview until all six cutouts are usable. Then choose **Move joints** to place the bend dots at elbows, knees and other joints. The two dot tools do different jobs: cutout dots define the paper pieces; joint dots define where those pieces bend. If a save finds a missing or too-small shape, it switches to **Trace cutouts** and selects the first part to fix. This is an assisted workflow: faint pencil, shadows and overlapping limbs often need corrections or an adult's help. It is not automatic pose recognition. Wings, horns, clothes and tails can travel with a chosen body part; they do not gain separate creature locomotion.
