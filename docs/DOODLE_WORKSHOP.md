...
3. Draw the head, body, two arms and two legs. **Fighter color** changes the starter body's color; **Pen color** separately chooses the color for new strokes. Existing marks keep their own colors. Use Details to attach extra marks to the chosen part. Three pen sizes, a drag eraser, undo/redo, guide toggle and zoom are available.
...
Frame the paper with the four corner dots, adjust **Page cleanup** strength, and use **Keep** and **Erase** to correct the transparent preview. The small cleanup preview updates as you change the slider; the large canvas keeps the source visible while you place its corners. Cleanup removes light, low-color paper pixels, including warm gray from shaded photos; saturated marker colors stay visible. Keep and Erase strokes apply in the order you draw them, so a later Keep can restore a pale mark and a later Erase can remove it again. The adjacent **White edge** switch adds the sticker border around your cutout, so it is separate from page cleanup. This step does not detect limbs or make cutouts for you. In **Bring to life**, choose **Trace cutouts** and tap around the outside edge of each body part (head, body, two arms and two legs). Use enough points to follow the whole part; tiny or nearly straight slivers are rejected. The full source photo stays visible on the main canvas until all six cutouts are usable. Then choose **Move joints** to place the bend dots at elbows, knees and other joints. The two dot tools do different jobs: cutout dots define the paper pieces; joint dots define where those pieces bend. If a save finds a missing or too-small shape, it switches to **Trace cutouts** and selects the first part to fix. This is an assisted workflow: faint pencil, shadows and overlapping limbs often need corrections or an adult's help. It is not automatic pose recognition. Wings, horns, clothes and tails can travel with a chosen body part; they do not gain separate creature locomotion.
