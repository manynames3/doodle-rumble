...
const OUT := "res://../docs/test-results/v072/"
...
func _save_viewport(viewport: SubViewport, filename: String) -> void:
	...
	if error == OK: print("CUSTOM_NATIVE_CAPTURE ",OUT+filename)
...
func run() -> void:
	...
	check(source.save_png(OUT+"synthetic_photo_source.png") == OK,"reviewable original source saved")
	...
	var file := FileAccess.open(OUT+"custom_fighters_native_qa.txt",FileAccess.WRITE)
	if file != null:
