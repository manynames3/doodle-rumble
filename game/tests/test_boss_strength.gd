...
func run():
	...
	check(strength.damage("dark_lord",26) == 33 and strength.damage("h4ck3r",13) == 17 and strength.damage("red",20) == 20,"boss strength rounds fractional damage upward and leaves other fighters unchanged")
	game.countdown = 0
	game.first.reset_at(Vector2(400,599))
	game.second.reset_at(Vector2(680,599))
	var boss = game.second
	boss.start_attack(false)
	check(int(boss.attack_spec.damage) == 33,"Dark lord blade uses the shared 25 percent outgoing multiplier")
	boss.attack_time = -1
	boss.start_attack(true)
	check(int(boss.attack_spec.damage) == 40,"Dark lord special attack payload uses the shared multiplier")
	var wave = load("res://scripts/attack_effect.gd").new()
	game.world.add_child(wave)
	wave.configure(boss,"shockwave",-1)
	check(wave.damage == 40,"Dark lord moving ground wave scales once")
	wave.queue_free()
	boss.attack_time = -1
	...
	check(patterns.size() == 5,"Dark lord naturally cycles five distinct signaled patterns")
	check(shots.has("void_orb") and shots.has("shockwave"),"Dark lord launches void fan and ground waves")
	check(hazard_kinds.has("dark_rift"),"Dark lord raises his unique ground rift")
	...
	for kind in ["void_orb","pellet_fan"]:
		var totals = []
		for delta in [1.0/30,1.0/60,1.0/120]:
			...
			shot.queue_free()
		check(totals == ([30,30,30] if kind=="void_orb" else [23,23,23]),kind+" 25 percent boss damage is stable across fixed-step sampling")
	...
	check(game.first.health==75,"Dark lord rift deals one scaled 25-point hit")
	game.hazards.clear()
	game.first.reset_at(Vector2(450,599))
	...
	game.hazards.tick(0.02,[game.first,game.second])
	check(game.first.health==88,"Dark lord camera mark rounds its nine-point base payload to twelve")
